...
# Thunderbird Mail Merge recipient file for the "Student Email Account Details"
# campaign (replaces the manually-maintained `to_mail` files in Email_2025/).
#
# - `{ds}_to_email_1_2.csv` is now generated by the LDAP step (not here) — it
#   carries the SSO/LDAP passcode straight from the LDAP export so every
#   LDAP-new student gets the password that was actually provisioned in AD.
...
def run_google_pipeline(
    ...
    # and Email 2 so the emailed credentials match the account.
    ...
    return upload_df, reactivation_df, to_email_3_df, audit_info
