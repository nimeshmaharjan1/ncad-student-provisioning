...
def generate_library_export(df: pd.DataFrame) -> pd.DataFrame:
    """
    Generates a system-specific Library export DataFrame from the cleaned Quercus data.
    
    Requirements:
    - Input: cleaned Quercus dataframe
    - Output columns ONLY:
      - Term Email → email
      - Student Category → borrower_category
      - Status → status
    ...
    if missing_cols:
        raise KeyError(f"Required columns missing for Library export: {missing_cols}")
