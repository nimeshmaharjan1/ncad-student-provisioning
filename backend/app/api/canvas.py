...
async def export_canvas(baseline: UploadFile = File(...), quercus: UploadFile = File(...)):
    try:
        ...
        if check["missing_required"]:
            blocking = blocking_missing_required("canvas", check)
            if blocking and get_validation_mode("canvas") == "strict":
                logger.warning("Canvas export rejected — missing required Quercus columns: %s", blocking)
                return missing_required_response("canvas", {**check, "missing_required": blocking})
            logger.warning(
                "Canvas export proceeding — missing required columns auto-added as blank: %s",
