...
GOOGLE_QUERCUS_REQUIRED_COLUMNS = [
    "ID Number", "First Name", "Last Name",
]

GOOGLE_QUERCUS_OPTIONAL_COLUMNS: list[str] = []
...
async def export_google(baseline: UploadFile = File(...), quercus: UploadFile = File(...)):
    try:
        ...
        missing_required = [col for col in GOOGLE_QUERCUS_REQUIRED_COLUMNS if col not in quercus_cols]
        missing_optional = [col for col in GOOGLE_QUERCUS_OPTIONAL_COLUMNS if col not in quercus_cols]

        if missing_required:
            logger.warning("Google export rejected — missing required Quercus columns: %s", missing_required)
            return JSONResponse(
                status_code=422,
                content={
                    ...
                    "all_required": GOOGLE_QUERCUS_REQUIRED_COLUMNS,
                    "all_optional": GOOGLE_QUERCUS_OPTIONAL_COLUMNS,
                },
            )
