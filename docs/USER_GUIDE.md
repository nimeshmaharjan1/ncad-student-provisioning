...
| Recipient source | `YYYYMMDD_to_email_1_2.csv` (sent to the student's NCAD address) then `YYYYMMDD_to_email_3.csv` (sent to the student's home email) — both generated by the Google step |
| Mail Merge tool | Thunderbird **Tools → Mail Merge** |
...
> to maintain `to_mail` in `Email_2025/` by hand. `YYYYMMDD_to_email_1_2.csv`
...
> `YYYYMMDD_to_email_3.csv` carries the student's personal email (`email` —
> the Mail Merge recipient), `username`/`newemail` (the Term Email) and the
> Google temp password (`password`, the same UUID as the upload CSV). If any
> new student has no home email on record, the Google step shows an amber
> warning listing them — they are still included in `to_email_3` with a blank
> email column to fill in before merging.
