...
- `YYYYMMDD_email_new_students.csv` — recipient file for the Student Email Details campaign (see Phase 4, Email 3)
- `YYYYMMDD_date_to_email.csv` — per-student credentials to email out
...
| Recipient source | `YYYYMMDD_email_new_students.csv` (generated by the Google step) |
| Mail Merge tool | Thunderbird **Tools → Mail Merge** |

This sends each student their email account login details to their **personal email address**.

> The Google export now generates the recipient files automatically — no need
> to maintain `to_mail` in `Email_2025/` by hand. `YYYYMMDD_email_new_students.csv`
> contains the NCAD email (`Email Address [Required]`), the student's personal
> email (`Home Email` — pick this as the recipient column in Mail Merge), and
> the temporary password (`Temp`, the same one set on the Google account).
> `YYYYMMDD_date_to_email.csv` carries the credentials per student
> (`email`, `firstname`, `username`, `password`, `newemail`).
...
| `20260701_email_new_students.csv` | Mail Merge recipient file (Student Email Details campaign) |
| `20260701_date_to_email.csv` | Per-student email credentials |
