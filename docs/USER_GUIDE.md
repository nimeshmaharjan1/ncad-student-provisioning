...
> includes `Date of Birth`. This is expected and **does not trigger a warning**
> — LDAP exports include the `Date of Birth` column **blank** (the LDAP admin
> confirmed the column must remain, but empty is fine).
...
**The `.zip` contains:**
- `YYYYMMDD_ldap.csv` — new students (13 cols, includes **passcodes**)
- `pre_YYYYMMDD_ldap.csv` — save as your next baseline
- `YYYYMMDD_to_email_1_2.csv` — Mail Merge recipient file (`firstname`, `email`, `password`) for **Email 1** (SSO + Eduroam), one row per LDAP-new student; `password` is the real SSO passcode provisioned in AD
...
- `YYYYMMDD_to_email_3.csv` — Mail Merge recipient file (`email`, `firstname`, `username`, `password`, `newemail`) sent to personal addresses (Home Email + Google temp password). `YYYYMMDD_to_email_1_2.csv` for Email 1 is now delivered by the **LDAP step**, not here.
...
The system does **not** send emails. You use **Thunderbird with the Mail Merge add-on** for 2 email campaigns.

All instructions assume Thunderbird is set up with the NCAD email account (provided by IT support).

Both emails below are **only sent after Triangle confirms the students have
been added to the LDAP system** — do not send anything before that
confirmation.

#### Email 1: SSO Account & Eduroam Wi-Fi

| Setting | Value |
|---|---|
| Email template | **NCAD Important - SSO account and NCAD Eduroam Wifi - how to connect** |
| Recipient source | `YYYYMMDD_to_email_1_2.csv` (each student's NCAD address) |
| Mail Merge tool | Thunderbird **Tools → Mail Merge** |

This sends each student their SSO account details (username + passcode) and the
Eduroam Wi-Fi setup instructions.

> The LDAP export generates the recipient file automatically — no need to
> maintain `to_mail` in `Email_2025/` by hand. `YYYYMMDD_to_email_1_2.csv`
> (now in the LDAP ZIP) carries `firstname`, the student's NCAD address
> (`email`) and the student's real SSO/LDAP passcode (`password`) straight
> from that export — so emailed passwords always match AD.

#### Email 2: Google Apps for Education Account

| Setting | Value |
|---|---|
| Email template | **NCAD Google Apps for Education Account** |
| Recipient source | `YYYYMMDD_to_email_3.csv` (students' home emails) |
| Mail Merge tool | Thunderbird **Tools → Mail Merge** |

This sends each student their Google account login details for
`email@student.ncad.ie`.

> `YYYYMMDD_to_email_3.csv` uses the home email (`email`) as the Mail Merge
> recipient; `username`/`newemail` are the Term Email and `password` is the
> Google temp password (the same UUID as the upload CSV). If any new student
> has no home email on record, the Google step shows an amber warning listing
> them — they are still included in `to_email_3` with a blank email column to
...
| `20260701_to_email_3.csv` | Mail Merge recipient file (sent to home emails, Google credentials) |
