...
### Fix: Email 1 passwords now come from the LDAP export — and Email 1 is now delivered by the LDAP step
- **Bug:** `to_email_1_2.csv` carried freshly generated passcodes instead of
  ...
  `google_upload.csv`).
- **Second bug:** `to_email_1_2.csv` was sized to Google-new (e.g. 64) not
  LDAP-new (e.g. 67), so 3 LDAP-new students never got Email 1 at all.
- **Fix (interim in `a1b1469`):** the Google step required the LDAP export CSV
  and looked up passcodes from it. **Fix (now, Option A):** `to_email_1_2.csv`
  is generated by the LDAP step alongside `YYYYMMDD_ldap.csv` — one row per
  LDAP-new student, password straight from that export, so the count is always
  67 and the password always matches AD. The Google ZIP now contains only
  `google_upload.csv` + `google_reactivate.csv` + `to_email_3.csv` and the
  Google step is back to 2 uploads.
- Weekly order is unchanged: LDAP first, then Google — Email 1 is now
  available right after the LDAP export, before the Google export.
